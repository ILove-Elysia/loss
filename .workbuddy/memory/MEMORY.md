...
- **改 .gd → `test/gd_static_lint.py`；改 .tres/.tscn → `test/check_data_refs.py`；最后实机。编辑器开着别跑 headless**（抢 `.godot`）。Godot 4.7.2＝`E:\SteamLibrary\steamapps\common\Godot Engine\godot.windows.opt.tools.64.exe`；headless＝`godot --headless --path . --script res://test/<name>.gd`（仓库根＝`E:\GameMake\loss`，工程在 `loss-land`）；Git Bash 跑先补 `APPDATA`（否则 `user://` 落进项目内 `Godot/`，那是日志、已 gitignore）；**`--path` 只认 Windows 路径**。**编辑器开着也想 headless 验证：复制工程到临时目录 → 删副本 `.godot` 走全新导入 → 另设 `APPDATA` → `--path` 指副本**（⚠ 副本须先 `--editor --quit` 生成 `.godot` 再跑 `--script`，否则**所有** `class_name` 报 not declared、极易误判成"改动把类改坏了"；Steam 版是 GUI 子系统 ⇒ PowerShell 里 `& $godot` 不等进程、无 exit code、stdout 不进管道，**结果读 `logs/godot.log` 并轮询，跑完进程常残留需手动终止**）。
