...
- **Boss / 沙虫**：独立 `"boss"` 组、**绝不进 `"enemy"` 组**；永久世界状态只走 `WorldState`；状态机**没有 `dead`、终态 `GONE`**；换表**只在决策点**；技能全 CD 时**回 CHASE**；**领地/脱战判据＝玩家↔巢穴**（`player_home_distance()`，别与 `player_distance()` 混），追击/出招受**领地绳**约束、回家不受限。场地 `tscn/sandworm_test.tscn`（**F6 独立窗口**）＋回归 `test/boss_state_test.gd`（**90 断言**；13＝扣血同步重入、14＝受击窗口、15＝顺序轮转、16＝面前扇形 + 前摇预警片、17＝连吐 4 发的弹道、18＝面前长条矩形）。
