...
> **本文件＝高频铁律 + 领域指针（每轮自动注入，务必保持精简）**；细节、历史决策、踩坑过程一律在 **`MEMORY-archive.md`**（2026-09-26 由 18 份旧记忆合并，全量收录：细节详章、拆分前全文备份、2026-09-08～09-26 每日日志；参见其 `## 目录` 的 S01～S18 编号）。源文件备份于 `.workbuddy/archive/memory_backup_2026-09-26/`。**待办在 `E:\GameMake\loss\待办.md`**。

## 一、改文件 / 改数据的铁律
- `大纲.md`＝给人看的说明书：禁版本号/状态标记/代码名/字段名/待办/踩坑；未实装标「（规划中）」；改数值后回写。过程与接口契约 → `MEMORY-archive.md`。
...
- **删物品 / 移动 .gd 各需同步 5 处** → `MEMORY-archive.md` S02「文件同步清单」。**新增 `class_name` 要手工补 `.godot/global_script_class_cache.cfg`（lint 第 4 项查），或在用到处 `const X := preload("res://…")` 绕开缓存**。核心系统＝`class_name`+`static`，不用 autoload。
...
## 五、领域高频点（一句话触发；数值与细节 → `MEMORY-archive.md` S02）
