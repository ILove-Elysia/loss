...
#   ② 吐沙 = **弹道**（projectile_speed，射一发沙弹；躲的是**飞行段**，不是前摇）
#   ③④    = 就地圆形
#
...
#   projectile_speed        弹道初速（> 0 ⇒ 判定段射一发出去，命中归弹道自己）
...
func _build_attacks() -> void:
	...
	var shared: Array = [
		...
		# ---- ② 吐沙：冒头朝玩家吐一发**沙弹**（真弹道，靠走位躲） ----
		# 判定段把它射出去（18 m/s ≈ 3.6 倍玩家满速），命中与扣血都归沙弹自己。
		# 「躲的是飞行段，不是前摇段」：沙弹瞄的是**发射那一刻**玩家的位置，
		#   所以前摇里站哪都没用 —— 必须在沙弹飞到之前横移出命中半径（0.9 m）。
		#   越近越难躲：3 m ⇒ 只有 0.17 s ⇒ 约 0.85 m 的横移量；10 m 外就从容了。
