...
func _build_attacks() -> void:
	...
	# move_scale 2.5 ⇒ 名义突进速度 3.4 × 2.5 = 8.5 m/s；前摇只有 0.35 s。
	#   max_range = 0.5（2026-09-27 用户给定）：它要**贴到脸上**才扑 ——
	#   它是**表 2 的表头** ⇒ 这个 0.5 同时就是"表 2 的交锋距离"（见文件头的表头说明）。
	# ⚠⚠ 开了 facing_locked 之后**前摇其实是定身的**（此前这里的注释写反了，2026-09-27 修正）：
	#   facing_locked 时 _process_attack 既不 _face 也不更新 _move_dir，而 _move_dir
	#   每帧开头被重置为 ZERO ⇒ 那 8.5 m/s **没有位移出口**，前摇位移恒为 0。
	#   ⇒ 所以不存在"冲过头、玩家落到身后"的情形，判定原点就是它原地站着的位置。
	#   ⇒ 也意味着"快速靠近"实际发生在 CHASE 阶段（3.4 m/s），不在这一扑里。
	# ⚠ 0.5 m 起手的副作用：走廊半宽 1.3 m 比起手距离本身还大 ⇒ **贴脸几乎必中**
	#   （侧向分量最多也就 0.5 m），"长条 vs 圆形"在实战里分不出来。
	#   想让长条真正发挥作用（起手落在走廊中段、侧移 1.3 m 才谈得上躲），
	#   max_range 需要 ≥ 4。这条 2026-09-27 已列入待拍板，**没有擅自改**。
