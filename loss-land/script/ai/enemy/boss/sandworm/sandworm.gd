...
#
# **出手距离：① 2.0 / ② 15.0 / ③ 20.0 / ④ 0.5**（2026-09-27 用户给定）：
#   ⚠⚠ 这两张表的**表头都是近身招**（表 1 的 ① 要 2.0 m、表 2 的 ④ 要 0.5 m），
#     而轮转规则是"轮到的那招够不着 ⇒ `return null` 且**不推进指针**"（_pick_next_attack）。
#     两条合起来 ⇒ **实际交锋距离被表头锁死**：玩家不贴进 2 m，
...
func _build_attacks() -> void:
	...
	var shared: Array = [
		# ---- ① 撕咬：冒头咬一口（**只咬嘴前面的扇形**） ----
		# max_range = 2.0（2026-09-27 用户给定，前值 1.0）：**贴到跟前才咬**。
		# 它从远处游过来，只有玩家进了 2 m 才冒头（此前轮转卡在它身上、一直逼近）。
		# ⚠ 它是**表 1 的表头** ⇒ 这个 2.0 同时就是"表 1 的交锋距离"：
		#   玩家保持 2 m 以外 ⇒ 它一招都不放（见文件头的表头说明）。
		# ⚠ 判定半径仍是 4.5，**与 max_range 无关**：起手那一帧玩家在 2 m 内，
		#   之后前摇 0.9 s × 5.0 = 4.5 m 足够跑到扇形外 ⇒ 这一口仍然躲得掉。
		#   ⚠ 别把 max_range 抬到接近 4.5：起手就在判定边缘 ⇒ 玩家一步就出去，
		#     看起来就是"对着空气咬"（空放）。
		#   2.0 是"够得着，但还得在嘴前面"的那个距离：起手时玩家已经在扇形里
		#   （远小于半径 4.5）⇒ 一定能咬到；能不能躲掉完全看那 0.9 s 的走位。
