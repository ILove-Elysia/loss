...
##
## 参考：当前主相机 fov 50、base_distance 9.434、zoom 0.6~1.8，
##   俯角随缩放联动 30°~55°（推近放平 → 拉远抬高）。据此算出的可见地面：
##     · 身前：约 27 米（推近）→ 15 米（默认）→ 13 米（拉远）
##     · 两侧：约 ±5 米（推近）→ ±14 米（拉远）
##   注意"拉远"并不会看得更远 —— 抬高俯角吃掉了后退带来的纵深（见 camera_3d.gd）。
##   整个可见范围（含四个斜角）始终落在玩家周围 30 米以内，
##   70 米留了两倍以上余量，屏幕边缘不会"走到跟前才冒出来"。
##   （想省性能可以降到 40 米；但若把俯角调平，可见纵深会迅速变远，届时要一起改。）
##
