...
# 流程：
	...
	#   2. 按 tier 确定房间半径范围（TILE_SIZE=1、MAP=1600，绝对房间尺寸与放大方案一致）：
	#      - Tier 0（内圈/草原）：30-40 瓦片（小房间）
	#      - Tier 1（中圈）：46-56 瓦片（中等房间）
	#      - Tier 2（外圈/火山）：60-72 瓦片（大房间）
...
func _generate_blob_tiles(cx: float, cy: float, base_radius: int, rng: RandomNumberGenerator, irregularity: float = 0.5, branch_chance: float = 0.8, max_branches: int = 3) -> Array:
	...
	#
	# 用 4 条平行数组代替字典数组：GDScript 里 dict.x 是哈希查找，
	# 而"每格 × 每种子"的内层循环在大房间下可达数百万次，
	# 换成 PackedFloat32Array 索引后能快数倍。
	var num_seeds = 7 + rng.randi() % 6
	var seed_x := PackedFloat32Array()
	var seed_y := PackedFloat32Array()
	var seed_r := PackedFloat32Array()
	var seed_p := PackedFloat32Array()
	seed_x.resize(num_seeds)
	seed_y.resize(num_seeds)
	seed_r.resize(num_seeds)
	seed_p.resize(num_seeds)
	for s in range(num_seeds):
		...
		seed_x[s] = cos(angle) * offset
		seed_y[s] = sin(angle) * offset
		# 种子半径：0.3-1.1 倍基础半径（大小不一的种子）
		seed_r[s] = base_radius * (0.3 + rng.randf() * 0.8)
		# 种子相位：用于噪声计算（使每个种子形状不同）
		seed_p[s] = rng.randf() * 1000.0
	...
	var max_reach = 0.0
	for s in range(num_seeds):
		var reach: float = sqrt(seed_x[s] * seed_x[s] + seed_y[s] * seed_y[s]) + seed_r[s] * 1.25
		if reach > max_reach:
			max_reach = reach
	# 分支最大延伸距离
	...
	# --- 步骤 4：填充瓦片 ---
	#
	# 旧做法是"遍历整个包围方块，对每格逐个种子检测"。房间放大后这个方块
	# 会变得很大（分支最远可延伸到 4.3 倍半径），而真正属于房间的格子往往
	# 不到三成——绝大多数循环是在空转。
	#
	# 改为主动填充：每个种子/分支只扫描自己的影响范围，用一张覆盖图去重。
	# 工作量从 O(包围方块面积 × 种子数) 降到 O(实际覆盖面积)。
	var span: int = r_int * 2 + 1
	var grid := PackedByteArray()
	grid.resize(span * span)

	# 4a：种子——逐行扫描每个种子的噪声圆
	for s in range(num_seeds):
		var lim: float = seed_r[s] * 1.3  # _is_in_noisy_circle 的有效半径上限
		var sy0: int = maxi(int(floor(seed_y[s] - lim)), -r_int)
		var sy1: int = mini(int(ceil(seed_y[s] + lim)), r_int)
		for dy in range(sy0, sy1 + 1):
			var ddy: float = dy - seed_y[s]
			var half_sq: float = lim * lim - ddy * ddy
			if half_sq <= 0.0:
				continue
			var half: float = sqrt(half_sq)
			var sx0: int = maxi(int(floor(seed_x[s] - half)), -r_int)
			var sx1: int = mini(int(ceil(seed_x[s] + half)), r_int)
			var row_base: int = (dy + r_int) * span + r_int
			for dx in range(sx0, sx1 + 1):
				var idx: int = row_base + dx
				if grid[idx] != 0:
					continue  # 已被更早的种子/分支覆盖
				var sdx: float = dx - seed_x[s]
				if sdx * sdx + ddy * ddy > lim * lim:
					continue
				if _is_in_noisy_circle(sdx, ddy, seed_r[s], seed_p[s]):
					grid[idx] = 1

	# 4b：分支——扫描每个分支的包围盒
	for br in branches:
		var bbox: Array = _branch_bounds(br)
		var by0: int = maxi(int(floor(bbox[1])), -r_int)
		var by1: int = mini(int(ceil(bbox[3])), r_int)
		var bx0: int = maxi(int(floor(bbox[0])), -r_int)
		var bx1: int = mini(int(ceil(bbox[2])), r_int)
		for dy in range(by0, by1 + 1):
			var row_base: int = (dy + r_int) * span + r_int
			for dx in range(bx0, bx1 + 1):
				var idx: int = row_base + dx
				if grid[idx] != 0:
					continue
	...
	for dy in range(-r_int, r_int + 1):
		...
		for dx in range(-r_int, r_int + 1):
			if grid[row_base + dx] != 0:
				tiles.append({x = bx + dx, y = by + dy})
...
# 计算分支的包围盒（两段椭圆的外接矩形）
#
# 用途：把分支的逐格检测限制在必要范围内，避免扫描整个房间方块。
# 取两段椭圆的端点，再各向外扩一个"椭圆外接圆"半径（保守但够紧）。
#
# 返回：[x0, y0, x1, y1]（相对于房间中心的浮点坐标）
# ============================================================
func _branch_bounds(br: Dictionary) -> Array:
	var b_angle: float = br.angle
	var b_len: float = br.length
	var b_width: float = br.width
	var b_start: float = br.start_offset
	var bend_angle: float = br.bend_angle
	var bend_ratio: float = br.bend_ratio

	# 第一段起点、第二段起点（弯曲点）——与 _is_in_branch 中一致
	var sx1: float = cos(b_angle) * b_start
	var sy1: float = sin(b_angle) * b_start
	var mid_x: float = cos(b_angle) * (b_start + b_len)
	var mid_y: float = sin(b_angle) * (b_start + b_len)
	var b_len2: float = b_len * bend_ratio

	# 端点的外接范围
	var x0: float = min(sx1, mid_x)
	var x1: float = max(sx1, mid_x)
	var y0: float = min(sy1, mid_y)
	var y1: float = max(sy1, mid_y)

	# 椭圆在其外接圆内：半径取长半轴 + 短半轴（再留 1 格余量）
	var reach: float = max(b_len, b_len2) * 0.5 + b_width * 0.5 + 1.0
...
func _keep_largest_connected_component(tiles: Array) -> Array:
	...
	# 构建哈希表
	# 键用整数而非字符串：房间放大后瓦片数以万计，
	# str(x)+","+str(y) 的拼接与哈希在这里是显著热点。
	# 键 = (y+OFFSET) << SHIFT | (x+OFFSET)，OFFSET 用于容纳地图外的负坐标。
	var KEY_OFFSET: int = 4096
	var KEY_SHIFT: int = 16
	var tile_map = {}
	for tile in tiles:
		tile_map[((tile.y + KEY_OFFSET) << KEY_SHIFT) | (tile.x + KEY_OFFSET)] = tile

	var visited = {}
	var largest_component = []
	var queue: Array = []

	# 4 邻域在整数键上的位移（上下 = ±1 行，左右 = ±1 列）
	var ROW_STEP: int = 1 << KEY_SHIFT
	...
	for start_key in tile_map.keys():
		...
		while not queue.is_empty():
			var current_key: int = queue.pop_back()
			component.append(tile_map[current_key])

			# 4 个邻居的键（无需还原坐标再加减）
			var up: int = current_key + ROW_STEP
			var down: int = current_key - ROW_STEP
			var right: int = current_key + 1
			var left: int = current_key - 1

			if tile_map.has(up) and not visited.has(up):
				visited[up] = true
				queue.append(up)
			if tile_map.has(down) and not visited.has(down):
				visited[down] = true
				queue.append(down)
			if tile_map.has(right) and not visited.has(right):
				visited[right] = true
				queue.append(right)
			if tile_map.has(left) and not visited.has(left):
				visited[left] = true
				queue.append(left)
