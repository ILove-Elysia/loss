...
#   Tier 1: target_rx * 0.52  （中圈；target_rx=310 时约 161 瓦片）
#   Tier 2: target_rx * 1.0   （外圈；target_rx=310 时约 310 瓦片）
#
# 间距验证（确保不重叠，按 target_rx=310 计算）：
#   中圈 161 + 房间~56 ≤ 外圈 310 - 房间~72 → 217 ≤ 238 ✓
#   外圈 310 + 房间链延伸~86 + 房间72 + 沙滩44 = 512 < 地图半径 800 ✓
#
...
func normalize_to_tile_coords(nodes: Array, center: Vector2, target_rx: float, target_ry: float, rng: RandomNumberGenerator = null) -> Dictionary:
	...
	var tier_target_radius = {
		0: 0.0,                       # 内圈：锚点，直接用中心
		1: target_rx * 0.52,          # 中圈：~161 瓦片（target_rx=310）
		2: target_rx * 1.0,           # 外圈：~310 瓦片（target_rx=310）
	}
