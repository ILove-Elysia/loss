...
#     玩家在 22 m 外（**超出最远的 ③ 的 20 m**）整段时间一招都不放，走近 2 m 内又立刻出招
...
func _run_cases() -> void:
	...
	# 表 1 的表头 ① 现在要 2.0 m ⇒ 这里直接摆进 2 m 内，免得断言里混进
	# "它还要先追过来"那段与本需求无关的时间。
	_place_target_at(0.9)
	var fired_when_close: bool = await _wait_for_attack_phase(6.0)
	_check(fired_when_close, "贴近到 2 m 内 ⇒ 立刻恢复出招（出手距离是上界，不是把招式关掉）")
	_place_target_at(3.0)
	_target.revive()
	...
	# 收尾摆位：贴着 0.9 m ⇒ ① 够得着（它的 max_range 是 2.0）⇒ 会起手 ⇒ 顺带把
